...
<head>
    ...
    <title>Fiche d'impression des règlements</title>
    <style>
        #tout{
...
<body>
    {{-- <img src="/images/logo-INFAS.png" alt="infas" class="infas"> --}}<span id="dateImp">Imprimé le : {{ date('j'.'-'.'m'.'-'.'Y'.' : ') }}{{ date('H').' h'.' - '}}{{ date('i').' min'}}</span>    
        <h3>La liste des règlements enregistrés par agent responsable</h3>
    <table id="tout">
        <thead id="entete">
            <tr>
                <td>N° règlement</td>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $rcv->reg_date }}</td>
                <td>{{ $rcv->reg_mont }}</td>
                <td>{{ $rcv->reg_mrg_code }}</td>
                <td>{{ $rcv->reg_numREG }}</td>
